...
export const CategoriesPage = () => {
  const {
    ...
    handleAdd, handleEdit, handleDeleteClick, handleDeleteConfirm, openEdit, openAdd, t,
  } = useCategoriesPage();
  ...
  return (
    <div>
      ...
      {categories.length === 0 ? (
        ...
        <ul role="list" className="space-y-2" aria-label="Categories list">
          {categories.map((cat) => (
            <li key={cat.id}>
              <Card>
                <CardContent className="flex items-center gap-3 py-3 px-4">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button
                      ...
                      aria-label={t('categories.editAriaLabel', { name: cat.name })}
                      className="h-8 w-8"
                    >
                    ...
                    <Button
                      ...
                      aria-label={t('categories.deleteAriaLabel', { name: cat.name })}
                      aria-disabled={categories.length <= 1}
                      title={categories.length <= 1 ? t('categories.cannotDeleteLast') : undefined}
